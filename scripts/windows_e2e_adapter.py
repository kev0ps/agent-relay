#!/usr/bin/env python3
"""Windows process adapter for the shared native E2E harness."""
...
class WindowsE2EError(harness.E2EHarnessError):
...
def generate_credentials() -> tuple[str, str]:
    return harness.generate_credentials(WindowsE2EError)
...
class WindowsLifecycle(harness.Lifecycle):
    """Add Job Object and bounded diagnostic ownership to the shared lifecycle."""

    def __init__(self, job: WindowsJob | None = None) -> None:
        super().__init__(WindowsE2EError, "Windows E2E cleanup failed")
        self.job = job
    ...
    def add_cleanup(self, cleanup: Callable[[], None], *, label: str | None = None) -> None:
        super().add_cleanup(cleanup, label=label)
