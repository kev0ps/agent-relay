...
def make_handler(artifacts: Path, run_id: str) -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        server_version = "RelayDesktopFixture"
        sys_version = ""
        ...
        def _reply(
            self, status_code: int, body: bytes = b"", content_type: str | None = None
        ) -> None:
        ...
        def do_POST(self) -> None:
            ...
            if (
                ...
                or payload.get("event") != "applied"
                or not _valid_text(payload.get("value"))
            ):
                self._reply(400, INVALID, "application/json")
                return
            try:
...
def main() -> None:
    ...
    server = ThreadingHTTPServer((HOST, arguments.port), make_handler(artifacts, arguments.run_id))
