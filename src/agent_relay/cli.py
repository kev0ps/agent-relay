...
def _init_tools(args: argparse.Namespace, path: Path) -> list[str] | None:
    ...
    if args.tools is not None:
        ...
        invalid = [item for item in selected if item not in config.PUBLIC_TOOLS - {config.SERVER_LOCAL_TOOL}]
        if invalid:
    ...
    if args.scope == "agent" and sys.stdin.isatty():
        return config.select_tools_interactively({}, path)
    return None
...
def _run_config(args: argparse.Namespace, path: Path) -> int:
    if args.config_command == "init":
        ...
        tools = _init_tools(args, path) if args.scope == "agent" else None
        token = None
    ...
    if args.config_command == "set":
        ...
        if secret_key:
            ...
            config.set_value(path, args.scope, args.key, args.value)
    ...
    if args.config_command == "unset":
        ...
        return 0
    report = config.validate_document(path, args.scope)
    ...
    try:
        ...
        if args.command == "tools":
            if args.tool_command == "list":
                print(config.render_tools(path))
                return 0
            config.update_tool(path, args.tool, enabled=args.tool_command == "enable")
            print(f"{'enabled' if args.tool_command == 'enable' else 'disabled'} {args.tool}")
            return 0
        if args.command == "doctor":
            output, valid = config.doctor(path)
            print(output)
        ...
        if args.command == "agent":
            config.load_agent_settings(path)
